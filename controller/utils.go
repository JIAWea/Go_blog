...
func checkEmailExist(email string) string {
    if !vm.CheckEmailExist(email) {
        return fmt.Sprint("Email does not register yet! Please Check email.")
    }
    return ""
}
...
func checkUserExist(username string) string {
    if !vm.CheckUserExist(username) {
        return fmt.Sprintf("Username already exist, plz choose another username")
    }
    return ""
}
